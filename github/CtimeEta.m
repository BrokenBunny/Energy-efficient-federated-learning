function [opteta,opttimemar,optbanmar,sumban,etamar]=CtimeEta(Tcom,fre,pow,Akpar,apar,Kuser,PathLoss_User_BS,...
...
opttimemar=zeros(Kuser,1);
optbanmar=zeros(Kuser,1);
...
%                     (lambertw(temp2*exp(temp2))+temp2);
        ...
        etamar(idnum)=sum(banmar);
        if sumban>etamar(idnum)
            sumban=etamar(idnum);
            opttimemar=timemar;
            optbanmar=banmar;
            opteta=eta;
